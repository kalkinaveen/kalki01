...
const MembershipsPage = () => {
  ...
  return (
    <div className="pt-10 pb-20 eh-grid-bg">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="text-center mb-12">
          <div className="eh-kicker justify-center mb-3">// MEMBERSHIPS</div>
          <h1 className="eh-display text-4xl md:text-6xl font-black">JOIN THE <span className="eh-neon">UNDERGROUND</span></h1>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {MEMBERSHIPS.map(m => (
            <div key={m.id} className={`eh-panel eh-brackets p-7 relative eh-card ${m.popular ? 'eh-panel-glow' : ''}`} style={ m.popular ? { borderColor: 'rgba(0,255,157,.55)' } : {}}>
              ...
              <div className="flex items-baseline gap-1 my-3"><span className="eh-display text-5xl font-black eh-neon">${m.price}</span><span className="opacity-60 eh-mono">/{m.period}</span></div>

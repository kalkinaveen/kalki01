...
const ToolsPage = () => {
  ...
  const onDownload = (t) => { toast.success('Download queued', { description: `${t.name} · ${t.size}` }); };
  return (
    <div className="pt-10 pb-20">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="text-center mb-10">
          <div className="eh-kicker justify-center mb-3">// ARSENAL</div>
          <h1 className="eh-display font-black" style={{ fontSize: 'clamp(2rem, 6vw, 4rem)' }}>HACKING <span className="eh-neon">TOOLS</span></h1>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {config.tools.map(t => (
            <div key={t.id} className="eh-card eh-panel eh-brackets p-6">
              ...
              <div className="flex items-center justify-between eh-mono text-xs opacity-70 mb-4"><span>size : {t.size}</span><span>dl : {Number(t.downloads || 0).toLocaleString()}</span></div>

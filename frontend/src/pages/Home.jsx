...
import { BOOKS, MEMBERSHIPS, BLOGS } from '../mock';
import { Link } from 'react-router-dom';
import { ArrowRight, Check, FileText, Star, Lock, Zap, Calendar } from 'lucide-react';

const STATS_DATA = [
  { label: 'Active Clients', end: 12400, suffix: '+' },
  { label: 'Orders Delivered', end: 48920, suffix: '+' },
  { label: 'Years Experience', end: 7, suffix: '+' },
  { label: 'Avg. Rating', end: 49, suffix: '/50', divisor: 10 },
];
      ...
      {STATS_DATA.map(s => (
        <div key={s.label} className="text-center">
          <div className="eh-display font-black eh-neon" style={{ fontSize: 'clamp(1.6rem, 4vw, 2.6rem)' }}>
            {s.divisor ? <><CountUp end={s.end/s.divisor} />{s.suffix}</> : <CountUp end={s.end} suffix={s.suffix} />}
          </div>

...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md" style={{ background: 'rgba(5,6,8,.78)', borderBottom: '1px solid var(--eh-border)' }}>
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 py-3 flex items-center justify-between gap-2 sm:gap-4">
        <Link to="/" className="flex items-center gap-2 sm:gap-3 group min-w-0 flex-shrink">
          <Logo size={36} className="sm:w-[42px] sm:h-[42px] shrink-0" />
          <div className="block min-w-0">
            <div className="eh-brand font-black tracking-widest text-sm sm:text-base leading-tight truncate">
              <span className="eh-title-error">ERROR</span><span className="eh-title-hacker">HACKER</span>
            </div>
            <div className="hidden sm:block text-[9px] sm:text-[10px] eh-mono opacity-60 leading-tight truncate">// underground tech intel</div>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button onClick={() => setSearchOpen(true)} data-testid="nav-search-mobile" aria-label="search" className="md:hidden w-9 h-9 sm:w-10 sm:h-10 grid place-items-center rounded border border-[var(--eh-border)] hover:border-[var(--eh-green)] transition-colors"><Search size={16} /></button>
          <Link to="/cart" data-testid="nav-cart" className="relative w-9 h-9 sm:w-10 sm:h-10 grid place-items-center rounded border border-[var(--eh-border)] hover:border-[var(--eh-green)] transition-colors">
          ...
          <button onClick={toggle} aria-label="toggle theme" className="w-9 h-9 sm:w-10 sm:h-10 grid place-items-center rounded border border-[var(--eh-border)] hover:border-[var(--eh-green)] transition-colors">
            {theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}
          </button>
          <button onClick={() => setOpen(o => !o)} className="lg:hidden w-9 h-9 sm:w-10 sm:h-10 grid place-items-center rounded border border-[var(--eh-border)]" aria-label="menu">
      ...
      {open && (
        <div className="lg:hidden border-t border-[var(--eh-border)]">
          <div className="px-4 py-3 flex flex-col">
            ...
            {user && <button onClick={async () => { setOpen(false); await logout(); nav('/'); }} className="eh-mono py-3 text-sm tracking-widest uppercase border-b border-[var(--eh-border)] text-left">logout</button>}
          </div>
        </div>
      )}
